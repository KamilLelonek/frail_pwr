...
KamilLelonekHTNActorController::KamilLelonekHTNActorController( ActorAI* ai ) : IActorController(ai)
{
    ...
    m_planner -> init(
						getAI() -> getHtnMethodsPath(),
						getAI() -> getHtnOperatorsPath(),
						getAI() -> getHtnGoalsPath()
					 );
	...
	*********************/
    ...
    m_actions["opHeal"			]	= &KamilLelonekHTNActorController::actionHeal;
...
void KamilLelonekHTNActorController::onCreate() {
    ...
    m_planner -> setStateBool("IsActorHealthy",		true);
...
bool KamilLelonekHTNActorController::isActorHealthy()
{
...
{	
	if(getAI() -> isInPowerLake() && !KamilLelonekHTNActorController::isActorHealthy()) 
	{
		getAI() -> setSpeed(0.f);
	}
...
void KamilLelonekHTNActorController::updateWorldState(float dt) {
	m_planner -> setStateBool("IsEnemyAttack", m_isAttacked);
    m_planner -> setStateFloat("ActorHealth", getAI() -> getHealth());
    m_planner -> setStateBool("IsActorHealthy",	KamilLelonekHTNActorController::isActorHealthy());
...
void KamilLelonekHTNActorController::executeTask(HTN::pOperator nextTask) {
	...
	if(KamilLelonekHTNActorController::isActorHealthy())
	{
	...
	{
		KamilLelonekHTNActorController::actionHeal(nextTask -> getDuration());
	}
}
...
bool KamilLelonekHTNActorController::actionHeal(float duration)
{
	ActorAI* actor = getAI();
